...
//dynamic row
            ...
            child: DropdownButton(
              ...
              items: selectData.map((t) {
                return DropdownMenuItem(
                  child: Text(t["text"], style: const TextStyle(fontWeight: FontWeight.w500)),
                  value: t["value"],
                );
              }).toList(),
              onChanged: (newVal) {
  ...
  InputDecoration inputDecoration(String name, String fieldType) {
    return InputDecoration(
      ...
      filled: true,
      label: Text(name, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 17)),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: SafeArea(
        child: FutureBuilder(
          future: DefaultAssetBundle.of(context).loadString("assets/form_fields.json"),
          builder: (context, AsyncSnapshot snapshot) {
            if (snapshot.hasError) return const Text('Error in loading form data.');
            if (snapshot.data != null) {
              final jsonResult = jsonDecode(snapshot.data);
              if (jsonResult.isEmpty) return const Text('There is no data for the form.');
              return Container(
